...
parser.add_argument('--dataset', type=str, default='PaviaU', choices=dataset_names, help="PaviaU, IndianPines, Salinas")
...
parser.add_argument('--folder', type=str, help="Folder where to store the datasets.", default="./Datasets/")
parser.add_argument('--runs', type=int, default=1, help="Number of runs (default: 1)")
parser.add_argument('--patch_size', type=int, default=7, help="Input patch size")
...
parser.add_argument('--data_aug', action='store_true', default=True, help="If use data augmentation(是否使用数据增强)")

parser.add_argument('--cuda', type=str, default='0', help="Specify CUDA device")
...
# seeds = [7] * 10    #used for finding the best number of groups 用于寻找最佳组数

results = []
# run the experiment several times
for run in range(N_RUNS):
    np.random.seed(seeds[run])
    print("seed:", seeds[run])
    # Sample random training spectra 随机训练光谱样本
    train_gt, test_gt = sample_gt(gt, PERCENTAGE, seed=seeds[run])
    print("train_gt_init:", np.sum(train_gt > -1))
    print("test_gt_init:", np.sum(test_gt > -1))
    # Split train set in train/val
    train_gt, val_gt = sample_gt(train_gt, 0.5, seed=seeds[run])
    print("train_gt_end:", np.sum(train_gt > -1))
    print("val_gt_init:", np.sum(val_gt > -1))
    ...
    try:
        train(model, optimizer, criterion, train_loader, val_loader, EPOCH, model_dir, scheduler=scheduler,
              device=device)
